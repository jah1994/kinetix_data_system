...
subdirs = [s for s in subdirs if 'speed' in s ] # CHANGE THIS TO TOGGLE BETWEEN SENSITIVY AND SPEED MODE DIRECTORIES
subdirs = [s for s in subdirs if 'M11_Sept21_speed' not in s ] # Many false positives in this one
    ...
    for s in subdirs:
        ...
        for k in range(len(sources)):
            sim_params[k] = np.array([sources[k], # one event per light curve
                                    np.random.randint(5000, X.shape[0] - 5000, 1),
