...
rx, ry, sigma_x, sigma_y, xc_ref, yc_ref, psf_model = imageproc.update_r(ref, peaks, r=r, nsigma=config.nsigma, lim=config.lim, img_path=img_path)
    ...
    while n < N:
        ...
        try:
            ...
            if ONLINE is True:
                ...
                if SCENE == 0 and batch >=1:
                    #data = np.flip(data)
                    #data = np.random.normal(0,1, data.shape)
                    data = shift(data, (-40, -10), order=0, cval=np.median(data))
            ...
            if n % (pf - 1) == 0 and n != 0 and config.real_time_plot is True:
                ...
                # check aperture stamp has not shifted badly in time since reference exposure frame was acquired
                if len(med_stamp1_fluxes) <= config.burn_in:
                    ...
                    print(abs(xc_stamp_centrality - 0.5), abs(yc_stamp_centrality - 0.5))
