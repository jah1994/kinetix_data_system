...
try:
    ...
    logging.info('Unable to locate PyVCAM module... camera functionality not available.')
...
rx, ry, sigma_x, sigma_y, psf_model = imageproc.update_r(ref, peaks, r=r, nsigma=config.nsigma, lim=config.lim)
...
## brings up an interactive plot, allowing user to manually drag boxes to define
## regions in which to compute background sky level estimates
logging.info('Please select background regions...')
...
for batch in range(batches):
    ...
    if ONLINE is False:
        ...
        img_coll = np.array(MultiImage(file_name))
