...
dirs = [s for s in dirs if 'speed' in s ] # CHANGE THIS TO TOGGLE BETWEEN SENSITIVY AND SPEED MODE DIRECTORIES
dirs = [s for s in dirs if 'M11_Sept21_speed' not in s ]
...
files_res = [f for f in files_res if 'speed' in f ] # CHANGE THIS TO TOGGLE BETWEEN SENSITIVY AND SPEED MODE DIRECTORIES
files_sim_params = [f for f in files if 'sim_params' in f ]
files_sim_params = [f for f in files_sim_params if 'speed' in f ]
...
for x in range(xmin, xmax):
    ...
    for dir in dirs:
        ...
        if (f_res.split('res' + str(x))[1] != '.npy'):
            continue

        print('x:', x)
        print('Data directory:', dir)
        print('Files:', f_res, f_param)
        print(f_res.split('res' + str(x))[0], f_param.split('_sim_params' + str(x))[0])
